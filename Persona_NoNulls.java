...
public class Persona_NoNulls{
    ...
    public static Peers[] process(int min, int max, Persona[] arr, Counter filt, Counter ins){
        ...
        /* Relleno con personas llamadas empty y numeros negativos,
        *  Hecho así para no tener que tratar NullPointerExceptions
         */
        // TODO k (nombres distintos)
        // t2_comps = nanosegundos de FILTRADO
        return processed;
    }

    public static void sort(Peers[] v, int len, Counter counter){
        long t0 = System.nanoTime();
        sortRec(v, 0, len, counter);
    ...
    public static int partition(Peers[] v, int low, int hi, Counter counter){
        ...
        // System.out.printf("comps:%d, movs:%d\n",comps,movs);
        return lim - 1;
    }

    public static void main(String[] args){
        Persona[] persons = new Persona[0];
        try{
            persons = leeFichero("personas_va.txt");
        }
        catch(IOException e){
        ...
        //TODO comp es el numero de comparaciones para encontrar el nombre
        ...
        Counter extraction = new Counter();
        Peers[] data = process(datemin, datemax, persons, filtrado, insercion);
        ...
        sort(data, size, extraction);
        long sort_time = System.nanoTime();
        long search_time;
        for (int i = 0; i < 5; i++) {
